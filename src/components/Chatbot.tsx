    ...
    <>
      ...
      {!isOpen && (
        <div
          ...
          className="fixed bottom-5 right-5 bg-rose-500 text-white p-4 rounded-full shadow-lg cursor-pointer hover:bg-rose-600 transition duration-300 ease-in-out"
        >
        ...
        <div className="fixed bottom-5 right-5 bg-white p-4 rounded-lg shadow-lg w-80 max-w-full transition-all duration-300 ease-in-out">
          <div className="overflow-y-auto max-h-60 mb-3">
            {messages.map((message, index) => (
              <div key={index} className={`mb-2 ${message.sender === 'user' ? 'text-right' : ''}`}>
                <div
                  className={`inline-block px-4 py-2 rounded-lg ${message.sender === 'user' ? 'bg-rose-500 text-white' : 'bg-rose-100'}`}
                >
                  {message.text}
                </div>
              </div>
          ...
          <div className="flex">
            ...
            <button
              ...
              className="ml-2 px-4 py-2 bg-rose-500 text-white rounded-lg hover:bg-rose-600 focus:outline-none focus:ring-2 focus:ring-rose-500"
            >
              Send
            </button>
          </div>
