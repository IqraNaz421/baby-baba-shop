...
const Cart: React.FC = () => {
  const [cartItems, setCartItems] = useState<CartItem[]>([]); // Cart items type specified
  const [totalPrice, setTotalPrice] = useState<number>(0);
  const router = useRouter();
  ...
  useEffect(() => {
    ...
    const total = cart.reduce(
      (acc: number, item: CartItem) => acc + item.discountPrice * item.quantity,
      0
    );
    setTotalPrice(total);
  }, []);
  ...
  // Handle removal of an item from the cart
  const removeItem = (productId: number) => {
    const cart = JSON.parse(localStorage.getItem('cart') || '[]'); // Change 'let' to 'const'
    const updatedCart = cart.filter((item: CartItem) => item.id !== productId); // Remove item with matching ID
    localStorage.setItem('cart', JSON.stringify(updatedCart));
    setCartItems(updatedCart); // Update state to re-render
    updateTotalPrice(updatedCart); // Recalculate total price
  };
        ...
        {cartItems.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {cartItems.map((item) => (
              <div
                key={item.id}
                className="bg-white p-6 rounded-lg shadow-lg flex flex-col items-center border border-gray-200 hover:shadow-xl transition-all duration-300"
              >
                <Image
                  src={item.image}
                  alt={item.name}
